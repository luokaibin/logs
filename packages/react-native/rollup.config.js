...
/**
 * 将 `@logbeacon/platforms/<子路径>` 解析到 `packages/platforms` 源码（根据 platforms 的 package.json `exports`）。
 * 必须放在 `@rollup/plugin-node-resolve` 之前。
 */
function createResolveLogbeaconPlatformsPlugin() {
  const platformsPkgPath = path.resolve(__dirname, '../platforms/package.json');
  const platformsDir = path.dirname(platformsPkgPath);
  /** @type {Record<string, string>} */
  const subpathToFile = {};
  try {
    const pkg = JSON.parse(readFileSync(platformsPkgPath, 'utf8'));
    for (const [exportKey, val] of Object.entries(pkg.exports || {})) {
      if (exportKey === '.') continue;
      const sub = exportKey.replace(/^\.\//, '');
      const target =
        typeof val === 'string'
          ? val
          : val['react-native'] || val.import || val.default;
      if (typeof target !== 'string') continue;
      subpathToFile[sub] = path.resolve(platformsDir, target);
    }
  } catch {
    // 若读取失败则仅跳过本插件解析
  }
  ...
  return {
    name: 'resolve-logbeacon-platforms',
    resolveId(id) {
      if (!id.startsWith('@logbeacon/platforms/')) return null;
      let sub = id.slice('@logbeacon/platforms/'.length);
      if (sub.endsWith('.js')) sub = sub.slice(0, -3);
      return subpathToFile[sub] ?? null;
    },
  };
...
const resolveLogbeaconPlatformsPlugin = createResolveLogbeaconPlatformsPlugin();

const RN_CORE_LOG_AGGREGATOR_PLACEHOLDER =
  'logbeacon-internal:core-log-aggregator';

/**
 * `common/LogAggregator.js` 占位导入：按变体解析到 `@logbeacon/core/LogAggregator-sls` 或 `LogAggregator-loki`。
 * 必须放在 `resolve-logbeacon-core` 之前。
...
export default [
  {
    ...
    plugins: [
      createRnCoreLogAggregatorPlaceholderPlugin('sls'),
      resolveLogbeaconCorePlugin,
      resolveLogbeaconPlatformsPlugin,
      resolveIdbPlugin,
      resolvePlugin,
      commonjsPlugin,
      terserPlugin,
  ...
  // service worker
  {
    ...
    // 使用上面定义的插件数组
    plugins: [
      createRnCoreLogAggregatorPlaceholderPlugin('loki'),
      resolveLogbeaconCorePlugin,
      resolveLogbeaconPlatformsPlugin,
      resolveIdbPlugin,
      resolvePlugin,
